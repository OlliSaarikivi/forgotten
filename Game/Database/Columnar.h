...
private:
	...
	struct FreeColumn {
		Columnar& columnar;
		template<class Task> void operator()(const T& type) {
			using PointerType = AsPointer<Task::type>;
	...
	struct ReallocColumn {
		...
		template<class Task> void operator()(const T& type) {
			using PointerType = AsPointer<Task::type>;
			auto& column = get<PointerType>(columnar.columns);
			column = (PointerType)realloc(column, columnar.capacity_ * sizeof(Task::type));
		}
	};
...
public:
	...
	template<class Task> auto colBegin() const {
		return get<Task*>(columns);
	}
	auto begin() {
		return Iterator<TValues...>{ colBegin<TValues>()... };
	}

	template<class Task> auto colEnd() const {
		return colBegin<Task>() + size_;
	}
	auto end() {
...
public:
	template<class Task> auto colBegin() {
		return get<array<Task, N>>(columns).data();
	}
	template<class Task, class... Ts> auto begin() {
		return Iterator<Task, Ts...>{ colBegin<Task>(), colBegin<Ts>()... };
	}
	auto begin() {
		return begin<TValues...>();
	}

	template<class Task> auto colEnd() {
		return colBegin<Task>() + N;
	}
	template<class Task, class... Ts> auto end() {
		return Iterator<Task, Ts...>{ colEnd<Task>(), colEnd<Ts>()... };
	}
	auto end() {
		return end<TValues...>();
	}
