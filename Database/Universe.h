...
template<class TId, class... TComponents> class Universe {
	...
	template<class Task> struct WrappedNameOf;
	...
	template<class Task> struct ComponentTable;
	...
	template<class TMakeJoin, class Task> struct ExtendMakeJoin;
	template<size_t... Indices, class Task> struct ExtendMakeJoin<MakeJoin<Indices...>, Task> {
		using type = MakeJoin<Indices..., Task::value>;
	};

	template<class TMakeJoin, class Task> struct ExtendMakeOuterJoin;
	template<size_t... Indices, class Task> struct ExtendMakeOuterJoin<MakeOuterJoin<Indices...>, Task> {
		using type = MakeOuterJoin<Indices..., Task::value>;
	};

	template<class TMakeJoin, class Task> struct ExtendMakeAntiJoin;
	template<size_t... Indices, class Task> struct ExtendMakeAntiJoin<MakeAntiJoin<Indices...>, Task> {
		using type = MakeAntiJoin<Indices..., Task::value>;
	};
	...
	struct Eraser {
		...
		struct DoErase {
			...
			template<class Task> void operator()(const Task& type) {
				get<typename Task::type>(universe.tables).rows().erase(universe.toErase.begin(), universe.toErase.end());
			}
		};
